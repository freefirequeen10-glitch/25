...
// Dynamically managed Room Credentials modal called from history.js
window.openRoomModal = function(tournamentId) {
  ...
  if (isJoined && tourn.showRoomDetails === true && tourn.roomId) {
    vPanel.innerHTML = `
      ...
      <div class="bg-black/60 p-4 rounded-xl border border-gold/15 space-y-3 text-left font-grotesk">
        <div class="flex justify-between items-center border-b border-gold/10 pb-2">
          <div>
            <span class="text-[9px] text-slate-500 uppercase block">Room ID</span>
            <strong class="text-white font-mono text-sm tracking-wider">${tourn.roomId}</strong>
          </div>
          <button onclick="window.copyValue('${tourn.roomId}', 'Room ID')" class="px-2.5 py-1 bg-[#d4af37]/10 hover:bg-[#d4af37] text-[#d4af37] hover:text-black rounded-lg border border-[#d4af37]/30 text-[10px] font-bold uppercase transition-all">📋 Copy</button>
        </div>
        <div class="flex justify-between items-center pt-1">
          <div>
            <span class="text-[9px] text-slate-500 uppercase block">Password</span>
            <strong class="text-white font-mono text-sm tracking-wider">${tourn.roomPassword || "No Password"}</strong>
          </div>
          <button onclick="window.copyValue('${tourn.roomPassword || ''}', 'Password')" class="px-2.5 py-1 bg-[#d4af37]/10 hover:bg-[#d4af37] text-[#d4af37] hover:text-black rounded-lg border border-[#d4af37]/30 text-[10px] font-bold uppercase transition-all">📋 Copy</button>
        </div>
      </div>
    `;
    ...
    hPanel.innerHTML = `
      ...
      <div class="flex justify-center gap-6 text-xs text-slate-500 font-semibold uppercase tracking-wider py-1 font-mono">
        <span>ID: HIDDEN 🔒</span>
        <span>PW: HIDDEN 🔒</span>
      </div>
      <p class="text-xs text-slate-400 font-medium">Room ID and Password will be displayed here once enabled by the Admin before the match starts.</p>
    `;
...
export function updateMatchDetailsModalContent(tournamentId) {
  ...
  setSafeText('md-popup-mode', t.mode || "Solo");
  ...
  setSafeText('md-popup-winners', t.winners || "1st Place Only");

  const imgEl = document.getElementById('md-popup-banner');
  if (imgEl) imgEl.src = bannerSrc;

  // Retrieve matching admin configurations supporting multi-key definitions
      ...
      if (t.showRoomDetails === true && t.roomId) {
        ...
        bodyHtml = `
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs pt-1">
            <div class="bg-purple-950/40 p-3 rounded-xl border border-purple-500/15 flex justify-between items-center">
              <div>
                <span class="text-slate-500 font-semibold text-[9px] uppercase block">Room ID</span>
                <strong class="text-white font-mono text-sm tracking-wider">${t.roomId}</strong>
              </div>
            ...
            <div class="bg-purple-950/40 p-3 rounded-xl border border-purple-500/15 flex justify-between items-center">
              <div>
                <span class="text-slate-500 font-semibold text-[9px] uppercase block">Password</span>
                <strong class="text-white font-mono text-sm tracking-wider">${t.roomPassword || "No Password"}</strong>
              </div>
...
export function renderMatches() {
  ...
  filtered.forEach(t => {
    ...
    const winners = (t.winners || '1st Place Only');
    ...
    // Formats actual keys with secure copy functionality
    let roomCredsHTML = '';
    if (isJoined) {
      if (t.showRoomDetails === true && t.roomId) {
        roomCredsHTML = `
          <div class="flex items-center gap-1.5 text-white">
            <span class="bg-purple-950/60 px-1.5 py-0.5 rounded text-[10px] font-mono border border-purple-500/20">ID: ${t.roomId}</span>
            <span class="bg-purple-950/60 px-1.5 py-0.5 rounded text-[10px] font-mono border border-purple-500/20">PW: ${t.roomPassword || "No PW"}</span>
            <button onclick="event.stopPropagation(); window.copyValue('${t.roomId}', 'Room ID')" class="text-gold hover:text-white transition-colors ml-1 text-[10px] font-bold">📋 Copy ID</button>
            <button onclick="event.stopPropagation(); window.copyValue('${t.roomPassword || ''}', 'Password')" class="text-gold hover:text-white transition-colors ml-1 text-[10px] font-bold">📋 Copy PW</button>
          </div>
        `;
      } else {
        roomCredsHTML = `
          <div class="flex items-center gap-1.5 text-slate-500 text-[10px] font-mono">
            <span>ID: HIDDEN 🔒</span>
            <span>PW: HIDDEN 🔒</span>
          </div>
        `;
    ...
    card.innerHTML = `
      ...
      <div class="match-card-body p-3.5 flex-1 flex flex-col justify-between gap-2.5">
        ...
        <!-- Stats Grid (Prize Pool, Per Kill, Entry Fee, Winners) -->
        <div class="grid grid-cols-4 gap-1 p-2 bg-black/40 border border-purple-500/10 rounded-xl text-center">
          ...
          <div class="border-l border-purple-500/10">
            <span class="block text-[8px] uppercase tracking-wider text-slate-500 font-bold leading-none">Winners</span>
            <span class="block font-rajdhani font-extrabold text-[11px] sm:text-xs text-blue-400 mt-1 truncate" title="${winners}">${winners}</span>
          </div>
        </div>
